...
- **`.spatialaudio playat <sound> <pos> [range] [volume] [pitch]`** plays a sound at a place through
  the game's own `PlaySoundAt`, not straight through the engine as `.spatialaudio play` does, so the
  range check, fall-off, occlusion, reflections and pathing all apply. The position takes `~`relative,
  `=`absolute, or a selector (`l[]` for what you are looking at, `e[type=wolf*]` for the nearest
  wolf). It reports the distance and whether the game started the sound at all, which is the
  question when asking how far something carries. For reference: a wolf's howl is range 110, its
  Walk footsteps 10 and Run 15; a bear's footsteps 15 and 25.
