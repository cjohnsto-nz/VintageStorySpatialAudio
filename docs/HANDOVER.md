...
- **Still to do for the debug views (Chris, 20 Sep 2026):** the overlays do not trace a path or a reflection back to the sound that made it. The list now numbers each sound, so the next step is to draw that number (or a per-sound colour) on the `sources`, `paths` and `rays` overlays.
