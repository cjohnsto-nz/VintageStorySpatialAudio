...
1. `scripts/fetch-deps.ps1` downloads Steam Audio 4.8.1 and doctest, checks their SHA-256 hashes and extracts them into `third_party/`.
2. Native: `cmake --preset <rid>`, then build, `ctest`, and install into `artifacts/native/<rid>/`.
...
5. Packaging: `artifacts/mod/` (a drop-in mod folder) and `artifacts/vssteamaudio_<version>.zip`.
...
- Builds and tests the managed code against the public dedicated-server package for reference assemblies.
