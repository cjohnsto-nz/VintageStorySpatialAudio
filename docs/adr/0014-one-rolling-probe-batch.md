...
- **Steam Audio 4.8.1 gotchas** (with ADR 0013's): a simulator's `maxOrder` sizes the pathing coefficients, and a run writes `pathingOrder`'s worth regardless. With `maxOrder` 0 the three directional coefficients are lost (the paths were omnidirectional until this was found); `maxOrder` must be at least the pathing order.
