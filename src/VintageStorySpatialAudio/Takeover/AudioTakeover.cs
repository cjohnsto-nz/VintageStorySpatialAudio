...
{
    private const string HarmonyId = "spatialaudio.takeover";
    ...
    private AudioTakeover(ICoreClientAPI api, ILogger logger, AudioSession session, Members members, SpatialAudioConfig config)
    {
        this.api = api;
        this.logger = logger;
    ...
    /// vanilla audio then.
    /// </summary>
    public static AudioTakeover Begin(GameAssemblies game, AudioEngine engine, SpatialAudioConfig config, ICoreClientAPI api, ILogger logger)
    {
    ...
    public ILoadedSound? CreateSound(SoundParams sound, AudioData data)
    {
        ...
        SpatialAudioSound created = session.CreateSound(sound, () => Resolve(meta, location), Math.Max(1, meta.Channels));
    ...
    private void OpenOutput(bool headphones)
    {
        ...
        logger.Notification(
            ...
            SpatialAudioModSystem.DescribeOutput(stats), stats.SampleRate, stats.Channels, stats.DevicePeriodFrames);
    }
    ...
    {
        if (members.IntroMusic.GetValue(null) is not ILoadedSound intro || intro is SpatialAudioSound || intro.IsDisposed || intro.HasStopped)
        {
            return;
        }
