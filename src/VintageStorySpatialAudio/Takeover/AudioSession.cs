...
public sealed class AudioSession : IDisposable
{
    ...
    private readonly Dictionary<ulong, SpatialAudioSound> byVoice = [];
    private volatile string? solo;
    private readonly List<SpatialAudioSound> pending = [];
    ...
    public SpatialAudioSound CreateSound(SoundParams soundParams, Func<AudioAsset?> resolveAsset, int channelsHint = 1)
    {
        var sound = new SpatialAudioSound(this, soundParams, resolveAsset, channelsHint);
    ...
    public void Pump()
    {
        ...
        foreach (SpatialAudioSound sound in waiting)
        {
            if (sound.TryActivate())
            {
                lock (gate)
    ...
    public void SetOrigin(int x, int y, int z)
    {
        ...
        SpatialAudioSound[] sounds;
        lock (gate)
        {
            sounds = [.. byVoice.Values];
        }

        foreach (SpatialAudioSound sound in sounds)
        {
            sound.Reposition();
        }
    }
    ...
    public string? DescribeVoice(ulong voice)
    {
        lock (gate)
        {
            return byVoice.TryGetValue(voice, out SpatialAudioSound? sound) ? sound.Params.Location?.ToShortString() : null;
        }
    }
    ...
    internal void Activated(SpatialAudioSound sound, ulong voice)
    {
        lock (gate)
        {
            byVoice[voice] = sound;
    ...
    internal void Unregister(SpatialAudioSound sound, ulong voice)
    {
        lock (gate)
        {
            if (voice != 0)
