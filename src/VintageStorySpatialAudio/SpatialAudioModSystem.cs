...
using VintageStorySpatialAudio.World;

namespace VintageStorySpatialAudio;
...
/// </summary>
public sealed class SpatialAudioModSystem : ModSystem, IDisposable
{
    ...
    private SpatialAudioConfig? config;
    ...
    public override void StartPre(ICoreAPI api)
    {
        ...
        capi = clientApi;
        SpatialAudioConfig config = LoadConfig(clientApi);
    ...
    public override void StartClientSide(ICoreClientAPI api)
    {
        ...
        if (engine is not null)
        {
            ...
            api.Event.RegisterRenderer(frameProbe, EnumRenderStage.Before, "spatialaudio-frames");
        ...
        api.ChatCommands.Create("spatialaudio")
            .WithDescription("Spatial Audio engine status and test playback")
            ...
            .BeginSubCommand("speakertest")
                .WithDescription("Noise from each 7.1.4 speaker position in turn, then overhead; '.spatialaudio speakertest stop' ends it")
                .WithArgs(parsers.OptionalWord("stop"))
                .HandleWith(args => WithEngine(() => SpeakerTestCommand(api, args[0] as string)))
            .EndSubCommand()
            .BeginSubCommand("solo")
                .WithDescription("Debugging: hear only the sounds whose name contains this (e.g. '.spatialaudio solo anvil'), listed and drawn with the legs of their way round; '.spatialaudio solo off' ends it")
                .WithArgs(parsers.OptionalWord("name"))
                .HandleWith(args => WithEngine(() => SoloCommand(args[0] as string)))
            .EndSubCommand()
            .BeginSubCommand("mute")
                .WithDescription("Debugging: silence one way a sound reaches you -- direct, paths or reflections (each toggles) -- to hear which one a leak comes by; '.spatialaudio mute off' restores all")
            ...
            .BeginSubCommand("scene")
                .WithDescription("The acoustic scene: status, sounds [page|off] (every sound and how it reaches you), report (how the listed sounds reach you, block by block, to the log), or wire|faces|bounds|sources|rays|paths|off (overlay, Ctrl+F7 cycles), radius N, legend, export (OBJ), reload (materials)")
                .WithArgs(parsers.OptionalWord("action"), parsers.OptionalWord("value"))
                .HandleWith(args => WithEngine(() => sceneTools is null
                    ? "The world scene is off (BuildWorldScene in " + SpatialAudioConfig.FileName + ")."
            ...
            .BeginSubCommand("reverb")
                ...
                .WithArgs(parsers.OptionalWord("action"), parsers.OptionalWord("value"))
                .HandleWith(args => WithEngine(() => sceneTools is null
                    ? "The world scene is off (BuildWorldScene in " + SpatialAudioConfig.FileName + "): nothing to reflect off."
    ...
    private void StartWorldScene(ICoreClientAPI api, SpatialAudioConfig config)
    {
        ...
        api.Event.LevelFinalize += () =>
        {
            ...
            {
                Mod.Logger.Error("Spatial Audio: the world scene could not start: {0}", ex.Message);
            }
        };
    }

    private StatusReport TakeOver(SpatialAudioConfig config, ICoreClientAPI api, StatusReport report)
    {
        if (!config.TakeOverGameAudio)
        {
            return report with { TakeoverNote = $"TakeOverGameAudio is off in {SpatialAudioConfig.FileName}" };
        }
        ...
        {
            Mod.Logger.Error("Spatial Audio takeover failed; vanilla audio stays in charge. {0}", ex);
            takeover = null;
    ...
    private TextCommandResult WithEngine(Func<string> action)
    {
        ...
        {
            return TextCommandResult.Error("The Spatial Audio engine is not running; see .spatialaudio status.");
        }
        ...
        {
            Mod.Logger.Error("Spatial Audio command failed: {0}", ex.Message);
    ...
    private void TickPlayback()
    {
        ...
        {
            Mod.Logger.Error("Spatial Audio event polling failed: {0}", ex.Message);
        }
    }
    ...
    /// <summary>".spatialaudio solo [name|off]".</summary>
    private string SoloCommand(string? name)
    {
        ...
        if (name is null)
        {
            return session.Solo is null ? "Nothing is soloed. Usage: .spatialaudio solo <part of a sound's name>|off" : $"Soloed: '{session.Solo}'.";
        }
        ...
        return $"Only sounds with '{name}' in their name are heard. The panel lists them, and they alone are drawn: "
            ...
            + "'.spatialaudio mute direct|paths|reflections' silences one way at a time; '.spatialaudio solo off' ends this.";
    }

    /// <summary>".spatialaudio mute [direct|paths|reflections|off]".</summary>
    private string MuteCommand(string? way)
    {
        switch ((way ?? string.Empty).ToLowerInvariant())
        {
            ...
            case "off":
                ...
                break;
            case "":
                return "Muted: " + MutedText() + ". Usage: .spatialaudio mute direct|paths|reflections|off";
            default:
                return "Usage: .spatialaudio mute direct|paths|reflections|off";
        }
    ...
    /// <summary>".spatialaudio config [reset]".</summary>
    private string ConfigCommand(ICoreClientAPI api, string? action)
    {
        ...
        string path = Path.Combine(api.GetOrCreateDataPath("ModConfig"), SpatialAudioConfig.FileName);
        if (action is null)
        {
            return $"Settings: {path}\n" + string.Create(
                ...
                "Every value is written out in full; edit the file and restart. '.spatialaudio config reset' restores the defaults.";
        }

        if (!string.Equals(action, "reset", StringComparison.OrdinalIgnoreCase))
        {
            return "Usage: .spatialaudio config [reset]";
        }

        config.ResetToDefaults(EngineDefaults.Read());
        api.StoreModConfig(config, SpatialAudioConfig.FileName);
        Mod.Logger.Notification("{0} was reset to the defaults.", SpatialAudioConfig.FileName);
        return $"Settings reset to the defaults and written to {path}. Restart the game to apply them.";
    }

    /// <summary>
    /// ".spatialaudio perf [reset]". The report also goes to client-main.log: chat text cannot be
    ...
    private StatusReport BringUp(SpatialAudioConfig config, ILogger logger, string modConfigDirectory)
    {
        ...
        {
            logger.Error("Spatial Audio engine failed to start; vanilla audio remains active. {0}", ex);
            engine?.Dispose();
            engine = null;
    ...
    private SpatialAudioConfig LoadConfig(ICoreAPI api)
    {
        try
        {
            foreach (string carried in LegacyFiles.CarryOver(api.GetOrCreateDataPath("ModConfig")))
            {
                Mod.Logger.Notification("{0} was carried over from the mod's earlier name (vssteamaudio).", carried);
            }

            SpatialAudioConfig? config = api.LoadModConfig<SpatialAudioConfig>(SpatialAudioConfig.FileName);
            bool fresh = config is null;
            config ??= new SpatialAudioConfig();
            if (config.Migrate() && !fresh)
            {
                Mod.Logger.Notification("{0} was written by an earlier version; defaults that changed since were updated.", SpatialAudioConfig.FileName);
            }
        ...
        {
            Mod.Logger.Error("Could not read {0}; using defaults. {1}", SpatialAudioConfig.FileName, ex.Message);
            return new SpatialAudioConfig();
        }
    }
}
