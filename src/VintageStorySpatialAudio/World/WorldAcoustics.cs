...
/// <summary>What <see cref="WorldAcoustics"/> is doing, for the HUD and .spatialaudio scene.</summary>
...
internal sealed class WorldAcoustics : IDisposable
{
    ...
    public const string MaterialsOverrideFile = "spatialaudio-materials.json";
    ...
    private void LoadMaterials()
    {
        ...
        if (config is null)
        {
            config = capi.Assets.TryGet(new AssetLocation("spatialaudio", MaterialsAsset))?.ToObject<AcousticMaterialConfig>();
            materialSource = config is null ? "built-in defaults" : $"spatialaudio:{MaterialsAsset}";
        }
