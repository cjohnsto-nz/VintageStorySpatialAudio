...
/// <summary>User configuration, stored as ModConfig/spatialaudio.json.</summary>
/// <remarks>Engine bring-up and render options; quality presets arrive with the features they control.</remarks>
public sealed class SpatialAudioConfig
{
    ...
    /// <summary>Footsteps, whoever recorded them: 75 Hz takes the thump and leaves the step.</summary>
    ...
    /// <summary>
    /// Scales all reverb (0..4; also .spatialaudio reverb gain). 1 is the level Steam Audio
    ...
    /// <summary>Scales the early reflections, the first 0.1 s or so (0 = off, 0..4; also .spatialaudio reverb early).</summary>
    public float ReflectionEarlyGain { get; set; } = 1f;

    /// <summary>Scales the reverb tail after them (0 = off, 0..4; also .spatialaudio reverb tail).</summary>
    public float ReflectionTailGain { get; set; } = 1f;

    /// <summary>Part of a device name for the .spatialaudio play test command; empty = the device in use or the system default.</summary>
    ...
    {
        ArgumentNullException.ThrowIfNull(defaults);
        var fresh = new SpatialAudioConfig { ReflectionQuality = ReflectionQuality };
        foreach (System.Reflection.PropertyInfo p in typeof(SpatialAudioConfig).GetProperties())
        {
