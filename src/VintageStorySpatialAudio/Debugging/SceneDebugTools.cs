...
/// .spatialaudio scene commands.
/// </summary>
internal sealed class SceneDebugTools : IDisposable
{
    public const string HotkeyCode = "spatialaudio-scene-overlay";
    ...
    public SceneDebugTools(ICoreClientAPI capi, AudioEngine engine, WorldAcoustics world, System.Func<ulong, string?> describeVoice)
    {
        ...
        capi.Event.RegisterRenderer(renderer, EnumRenderStage.Opaque, "spatialaudio-scene");
        hud = new SceneHud(capi);
        hudListener = capi.Event.RegisterGameTickListener(_ => UpdateHud(), 100);
        capi.Input.RegisterHotKey(HotkeyCode, "Spatial Audio: cycle the acoustic scene overlay", GlKeys.F7, HotkeyType.DevTool, ctrlPressed: true);
    ...
    /// <summary>".spatialaudio scene [wire|faces|bounds|off|hud|radius N|legend|export|reload]".</summary>
        ...
        switch ((action ?? string.Empty).ToLowerInvariant())
        {
            ...
            case "sounds":
                ...
                return "Sound inspector on: the panel lists what you can hear, loudest first, and each one is drawn "
                    ...
                    + "to where it comes in), magenta only its reflections. '.spatialaudio scene sounds off' ends it.";
            case "off":
            ...
            case "radius":
                ...
                {
                    return "Usage: .spatialaudio scene radius <0-4> (chunks drawn around yours)";
                }
            ...
            case "export":
                ...
                string path = Path.Combine(folder, $"spatialaudio-scene-{DateTime.Now.ToString("yyyyMMdd-HHmmss", CultureInfo.InvariantCulture)}.obj");
            ...
            case "reload":
                ...
                return world.Reload();
            default:
                return "Usage: .spatialaudio scene [status|sounds [page|off]|wire|faces|bounds|sources|rays|paths|off|radius N|legend|export|reload]";
        }
    }

    /// <summary>".spatialaudio reverb [status|gain N|early N|tail N|rays]".</summary>
    public string ReverbCommand(string? action, string? argument)
    {
        switch ((action ?? string.Empty).ToLowerInvariant())
        {
            ...
            case "gain":
                ...
                {
                    return "Usage: .spatialaudio reverb gain <0-4> (1 = as simulated; ReflectionGain in the config sets it at start)";
                }
            ...
            case "tail":
                ...
                {
                    return $"Usage: .spatialaudio reverb {action!.ToLowerInvariant()} <0-4> (1 = as simulated, 0 = off)";
                }
            ...
            case "rays":
                ...
                return $"Overlay: {Describe(renderer.Overlay)}";
            default:
                return "Usage: .spatialaudio reverb [status|gain N|early N|tail N|rays]";
        }
    }
    ...
    /// <summary>
    /// ".spatialaudio scene sounds [page]": every sound that is sounding, loudest first, with which
    ...
    private string SoundsText()
    {
        ...
        text.Append(pages > 1 ? ", .spatialaudio scene sounds <page>)" : ")");
    ...
    /// <summary>
    /// ".spatialaudio scene report": everything about how the listed sounds are reaching the
    ...
    private string Report()
    {
        ...
        {
            capi.Logger.Notification("[spatialaudio report] {0}", line);
        }
    ...
    private string PathingText()
    {
        ...
        if (!p.Enabled)
        {
            return "Pathing: off (Pathing in " + Config.SpatialAudioConfig.FileName + ")";
        }
    ...
    private string ReflectionsText(bool detailed)
    {
        ...
        if (!r.Enabled)
        {
            return "Reflections: off (Reflections in " + Config.SpatialAudioConfig.FileName + ")";
        }
