...
internal sealed class TestPlayback(AudioEngine engine, ILogger logger, string? deviceName) : IDisposable
{
    ...
    public string ListDevices()
    {
        ...
        text.Append(deviceOpen ? $"In use: '{engine.GetStats().DeviceName}'." : "No device open yet (opened on the first .steamaudio play).");
    ...
    private string? EnsureDevice()
    {
        ...
        if (!string.IsNullOrWhiteSpace(deviceName))
        {
            ...
            if (device is null)
            {
                return $"No playback device matches '{deviceName}' (TestOutputDevice in {Config.SteamAudioConfig.FileName}). Try .steamaudio devices.";
            }
        }
