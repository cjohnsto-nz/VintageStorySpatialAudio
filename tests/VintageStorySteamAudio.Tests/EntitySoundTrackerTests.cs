...
using Body = VintageStorySteamAudio.Takeover.EntitySoundInference.Body;
...
public sealed class EntitySoundTrackerTests
{
    ...
    public void A_tracked_sound_follows_its_anchor_until_the_entity_is_gone()
    {
        ...
        SteamAudioSound sound = rig.Sound(new Vec3f(10, 20, 30));
        sound.Start();
    ...
    {
        using var rig = new Rig();
        SteamAudioSound stopped = rig.Sound(new Vec3f(0, 1, 0));
        SteamAudioSound disposed = rig.Sound(new Vec3f(0, 1, 0));
        SteamAudioSound playing = rig.Sound(new Vec3f(0, 1, 0));
        stopped.Start();
        stopped.Stop();
        disposed.Start();
        disposed.Dispose();
    ...
    private static void AssertPosition(float x, float y, float z, SteamAudioSound sound)
    {
    ...
    private sealed class Rig : IDisposable
    {
        ...
        public SteamAudioSound Sound(Vec3f position) =>
