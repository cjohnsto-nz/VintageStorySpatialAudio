...
public sealed class TakeoverUnitTests
{
    ...
    {
        Assert.Equal(SpatialMode.None, SteamAudioSound.Placement(new SoundParams { Position = null }).Mode);

        // Relative at the origin (UI, the player's own sounds): unpositioned.
        Assert.Equal(SpatialMode.None, SteamAudioSound.Placement(new SoundParams { Position = new Vec3f(), RelativePosition = true }).Mode);

        VoicePlacement relative = SteamAudioSound.Placement(new SoundParams { Position = new Vec3f(1, 2, 3), RelativePosition = true });
        ...
        VoicePlacement world = SteamAudioSound.Placement(new SoundParams { Position = new Vec3f(10, 64, -5), Range = 100 });
        Assert.Equal(new VoicePlacement(SpatialMode.World, 10, 64, -5, 8f), world);
        Assert.Equal(3f, SteamAudioSound.Placement(new SoundParams { Position = new Vec3f(1, 1, 1), Range = 16 }).MinDistance);
        Assert.Equal(1.5f, SteamAudioSound.Placement(new SoundParams { Position = new Vec3f(1, 1, 1), ReferenceDistance = 1.5f }).MinDistance);
    }
