...
public sealed class ConfigMigrationTests
{
    ...
    {
        var config = new SpatialAudioConfig { ReflectionGain = 1f };  // ConfigVersion 0: the old default, as stored
    ...
    {
        var chosen = new SpatialAudioConfig { ReflectionGain = 0.5f };
        Assert.True(chosen.Migrate());  // the version is stamped
        Assert.Equal(0.5f, chosen.ReflectionGain);

        var current = new SpatialAudioConfig { ConfigVersion = SpatialAudioConfig.CurrentConfigVersion, ReflectionGain = 1f };
    ...
    {
        var config = new SpatialAudioConfig { ReflectionRays = 1024, PathingMaxProbes = 400 };
