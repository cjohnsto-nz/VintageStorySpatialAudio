...
public sealed class ReflectionPresetTests
{
    ...
    public void Medium_is_the_engines_own_default()
    {
        ...
        Assert.Equal(10, medium.Sources);
        ...
        Assert.Equal(ReflectionQuality.Medium, new SpatialAudioConfig().ReflectionQuality);
        Assert.Equal(10, new SpatialAudioConfig().ToEngineOptions().ReflectionQuality.Sources);
    }
    ...
    public void Overrides_replace_single_values_and_are_clamped_to_what_the_engine_accepts()
    {
        ...
        var config = new SpatialAudioConfig { ReflectionQuality = ReflectionQuality.Low, ReflectionBounces = 12, Reflections = false, ReflectionGain = 9f };
        EngineOptions options = config.ToEngineOptions();
        Assert.False(options.Reflections);
        Assert.Equal(8, options.ReflectionQuality.Sources);
        Assert.Equal(12, options.ReflectionQuality.Bounces);
        Assert.Equal(4f, config.ReflectionGainClamped());
        Assert.Equal((1f, 0f), new SpatialAudioConfig { ReflectionTailGain = -2f }.ReflectionMixClamped());
        Assert.Equal(1f, new SpatialAudioConfig { ReflectionGain = float.NaN }.ReflectionGainClamped());
    }
