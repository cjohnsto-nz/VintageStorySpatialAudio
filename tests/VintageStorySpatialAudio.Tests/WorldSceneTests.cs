...
public sealed class WorldSceneTests
{
    ...
    {
        string path = Path.Combine(NativeTestEnvironment.RepoRoot(), "src", "VintageStorySpatialAudio", "assets", "spatialaudio", "config", "acousticmaterials.json");
...
public sealed class WorldSceneEngineTests
{
    ...
    public void A_snapshot_becomes_a_mesh_the_debug_view_can_read()
    {
        ...
        string obj = Path.Combine(Path.GetTempPath(), "spatialaudio-managed-scene.obj");
